...
<svelte:head><title>Edit Pengguna — Admin</title></svelte:head>

<section class="mx-auto max-w-2xl space-y-6 px-4 py-10">
	<header>
		<h1 class="h2">Edit Pengguna</h1>
		<p class="text-sm opacity-70">{data.email}</p>
	</header>
	...
	<form method="POST" use:enhance class="space-y-6">
		...
		<div class="card grid gap-4 p-6 sm:grid-cols-2">
			...
			<label class="label sm:col-span-2">
				<span>Password baru <span class="opacity-60">(kosongkan jika tidak diubah)</span></span>
				<input
					class="input"
					type="password"
					name="password"
