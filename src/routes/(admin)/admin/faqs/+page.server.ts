...
export const load: PageServerLoad = async (event) => {
	...
	const faqs = await faqsService.listFaqs(actor);

	return {
		faqs,
		canWrite: can(user.role as Role, 'create', 'faqs')
	};
};
...
export const actions: Actions = {
	delete: async (event) => {
		...
		try {
			await faqsService.deleteFaq(actor, id);
			return { deleted: true };
		} catch (err) {
			if (isAppError(err)) return fail(err.status, { message: err.message });
			throw error(500, 'Gagal menghapus FAQ');
		}
	}
};
