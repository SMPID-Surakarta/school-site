...
export const load: PageServerLoad = async (event) => {
	...
	const pages = await pagesService.listPages(actor);

	return {
		pages,
		canWrite: can(user.role as Role, 'create', 'pages')
	};
};
...
export const actions: Actions = {
	delete: async (event) => {
		...
		try {
			await pagesService.deletePage(actor, id);
			return { deleted: true };
		} catch (err) {
			if (isAppError(err)) return fail(err.status, { message: err.message });
			throw error(500, 'Gagal menghapus halaman');
		}
	}
};
