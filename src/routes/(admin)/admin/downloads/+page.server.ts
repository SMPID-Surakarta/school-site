...
export const load: PageServerLoad = async (event) => {
	...
	const downloads = await downloadsService.listDownloads(actor);

	return {
		downloads,
		canWrite: can(user.role as Role, 'create', 'downloads')
	};
};
...
export const actions: Actions = {
	delete: async (event) => {
		...
		try {
			await downloadsService.deleteDownload(actor, id);
			return { deleted: true };
		} catch (err) {
			if (isAppError(err)) return fail(err.status, { message: err.message });
			throw error(500, 'Gagal menghapus berkas');
		}
	}
};
