...
export const load: PageServerLoad = async (event) => {
	...
	const teachers = await teachersService.listTeachers(actor);

	return {
		teachers,
		canWrite: can(user.role as Role, 'create', 'teachers')
	};
};
...
export const actions: Actions = {
	delete: async (event) => {
		...
		try {
			await teachersService.deleteTeacher(actor, id);
			return { deleted: true };
		} catch (err) {
			if (isAppError(err)) return fail(err.status, { message: err.message });
			throw error(500, 'Gagal menghapus data guru');
		}
	}
};
